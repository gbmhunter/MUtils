...
# @brief 			Makefile for Linux-based make, to compile the MUtils library, example code and run unit test code.
# @details
...
src : deps $(SRC_OBJ_FILES)
	# Make library
	ar r libMUtils.a $(SRC_OBJ_FILES)
...
test : deps $(TEST_OBJ_FILES) | src
	...
	g++ $(TEST_LD_FLAGS) -o ./test/Tests.elf $(TEST_OBJ_FILES) -L./ -lMUtils $(DEP_LIB_PATHS) $(DEP_LIBS)
